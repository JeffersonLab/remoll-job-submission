...
// [process][ring][sector]
const int nMatrix=5;// Change this??
...
void deconvolution(){
  ...
  string fnms[nProcDef]={
		      ...
		      "../../../analysed_output/3mmoffset/moller.root",
		      "../../../analysed_output/3mmoffset/elastic.root",
		      "../../../analysed_output/3mmoffset/inelastic.root",
                      "../../../analysed_output/3mmoffset/inelastic.root",
                      "../../../analysed_output/3mmoffset/inelastic.root",
		      "../../../analysed_output/3mmoffset/elasticAl.root",
		      "../../../analysed_output/3mmoffset/quasielasticAl.root",
		      "../../../analysed_output/3mmoffset/inelasticAl.root",
		      "../../../analysed_output/3mmoffset/pion.root",
		      "byHand"
  };
...
void analyzeOne(int ring, int sect){
  ...
  for(int i=0;i<nMatrix;i++){
    ...
    printf("%15s %15.4f %15.4f %15.4f\n",procNm[i].c_str(),A[i][ring][sect],sigma[i],sigma[i]/A[i][ring][sect]);
  }
...
void readSim(string fnm,int proc, int addBkgnd){
  ...
  double rateFactor = 70.0/85.0*1.0/100.0;  
  double gfFactor = 1;
  if(proc >=5 && proc <= 6)
    gfFactor =1.0;
  if(proc<5)
  ...
  for(int i=0;i<6;i++)
	...
	A[proc][i][j] = hA->GetMean()*gfFactor;
      std::cout<< "Process " << proc<< " Ring " <<i << " Sector " <<j<<std::endl;
