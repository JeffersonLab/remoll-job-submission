int draw1Dene_sectorwise(TString input, TString generator){
...
for (Int_t i=0;i<g.size();i++){
   ...
   for(Int_t j=0;j<sector.size();j++){
        h[g[i]]=(TH1D*) f[g[i]]->Get("pr_"+sector[j]+"_ene");
        h[g[i]]->Scale(1.0/100);
...
hs.Draw("HISTnostack");
hs.SetMinimum(0);
hs.GetXaxis()->SetTitle("Incident energy of scattered electrons (MeV)");
hs.GetYaxis()->SetTitle("Rate(GHz/uA/sep/100MeV)");
legend->Draw("NDC");
c.Print(Form("1Dene_ring5_sectorwise_%s.png", generator.Data()));
return 0;
