int draw1Dth_sectorwise(TString input, TString generator){
...
for (Int_t i=0;i<g.size();i++){
   ...
   for(Int_t j=0;j<sector.size();j++){
        h[g[i]]=(TH1D*) f[g[i]]->Get("pr_"+sector[j]+"_th");
        h[g[i]]->Scale(1.0/100);
...
hs.Draw("HISTnostack");
hs.GetXaxis()->SetRangeUser(0,0.03);
hs.SetMinimum(0);
hs.GetXaxis()->SetTitle("Theta(rad)");
hs.GetYaxis()->SetTitle("Rate(GHz/uA/sep/0.3mrad)");
legend->Draw("NDC");
c.Print(Form("1DTheta_ring5_sectorwise_%s.png", generator.Data()));
return 0;
